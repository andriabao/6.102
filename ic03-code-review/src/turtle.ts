/* Copyright (c) 2007-2021 MIT 6.005/6.031 course staff, all rights reserved.
...
const DEGREES_TO_VERTICAL = CIRCLE_DEGREES / 4;
...
export class DrawableTurtle implements Turtle {
    ...
    public getSVG(): string {
        ...
        // converts an x or y coordinate to an SVG coordinate
        function coord(n:number):string { return n.toFixed(2); }

        // converts a PenColor to an SVG color
        function svgColor(color:PenColor):string { return PenColor[color].toLowerCase(); }

        const centerX = CANVAS_WIDTH/2;
        const centerY = CANVAS_HEIGHT/2;
        let svg = `<svg viewBox="0 0 ${coord(CANVAS_WIDTH)} ${coord(CANVAS_HEIGHT)}" xmlns="http://www.w3.org/2000/svg">
                   <g transform="translate(${coord(centerX)} ${coord(centerY)}) scale(1 -1)">\n`;
        for (const line of this.lines) {
            svg += (`<path fill="none" stroke="${svgColor(line.color)}"
