...
export enum IntegerGrammar {
    Expr, Sum, Primary, Constant, Whitespace, Product
}
...
function makeAbstractSyntaxTree(parseTree: ParseTree<IntegerGrammar>): IntegerExpression {
    ...
    } else if (parseTree.name === IntegerGrammar.Product) {
        // product ::= primary ('*' primary)*;
        const children = parseTree.childrenByName(IntegerGrammar.Primary);
        const subexprs = children.map(makeAbstractSyntaxTree);
        const expression: IntegerExpression = subexprs.reduce((result, subexpr) => new Times(result, subexpr));
        return expression;
    } else if (parseTree.name === IntegerGrammar.Sum) {
        // sum ::= product ('+' product)*;
        const children = parseTree.childrenByName(IntegerGrammar.Product);
        const subexprs = children.map(makeAbstractSyntaxTree);
